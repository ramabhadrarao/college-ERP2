...
async function checkAdminAccess() {
  try {
    ...
    try {
      ...
      console.log(
        `✅ Collections access: SUCCESS (${collectionsResponse.data.data.length} collections found)`
      );
    } catch (error) {
    ...
    try {
      await axios.post(
        `${config.directusUrl}/collections`,
        {
    ...
    try {
      const permissionsResponse = await axios.get(
        `${config.directusUrl}/permissions?filter[role][_eq]=${user.role}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      console.log(
        `✅ Permissions access: SUCCESS (${permissionsResponse.data.data.length} records found)`
      );
    } catch (error) {
    ...
    if (error.response?.status === 401) {
      console.error('❌ Unauthorized. Your session may have expired. Please re-authenticate.');
    } else {
